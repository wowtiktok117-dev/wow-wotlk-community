...
local mod = KRU:NewModule(misdirection)
...
KRU.defaults.profile.misdirections = defaults
...
local Addmisdirection, Resetmisdirections, Reportmisdirections
local misdirections, misdirectionFrames = {}, {}
local testmisdirections, testMode = {Diosussop = 20, Verethgan = 32, Sonsax = 6, Enary = 12}

KRU.options.args.misdirections = {
	type = "group",
	name = misdirection,
	order = 6,
	get = function(i)
	...
	args = {
		...
		reset = {
			...
			width = "double",
			confirm = function()
				return L:F("Are you sure you want to reset %s to default?", misdirection)
			end,
			func = function()
				KRU.db.profile.misdirections = defaults
				mod.db = KRU.db.profile.misdirections
				UpdateDisplay()
			end
		}
	}
...
function UpdateDisplay()
	...
	misdirections = testMode and testmisdirections or mod.db.misdirections

	for name, _ in pairs(misdirections) do
		local f = _G["KRUmisdirectionCounter" .. name]
		if f then
...
do
	...
	function CreateDisplay()
		...
		display = CreateFrame("Frame", "KRUmisdirectionCounter", UIParent)
		...
		t:SetScript("OnMouseUp", function(self, button)
			if not testMode and next(misdirections) and button == "RightButton" then
				menuFrame =
					menuFrame or CreateFrame("Frame", "KRUmisdirectionCounterMenu", display, "UIDropDownMenuTemplate")
...
function Addmisdirection(name)
	misdirections[name] = (misdirections[name] or 0) + 1
	rendered = nil
end

function Resetmisdirections()
	ResetFrames()
	mod.db.misdirections = {}
	rendered = nil
	UpdateDisplay()
end

function Reportmisdirections()
	if testMode then
		return
	end

	local list = {}
	for name, count in pairs(misdirections) do
	...
	SendChatMessage(misdirection, channel)
...
do
	...
	local function OnEvent(self, event, _, eventtype, _, srcName, _, _, _, _, _, spellname)
		if not self or self ~= display or event ~= "COMBAT_LOG_EVENT_UNFILTERED" then
			return
		elseif srcName and KRU:CheckUnit(srcName) and eventtype == "SPELL_CAST_SUCCESS" and spellname and spellname == misdirection then
			Addmisdirection(srcName)
		end
	end
...
function ResetFrames()
	for k, v in pairs(misdirectionFrames) do
		if _G[k] then
			_G[k]:Hide()
			_G[k] = nil
		end
...
function RenderDisplay()
	...
	local list = {}
	for name, count in pairs(misdirections or {}) do
		tinsert(list, {name, count})
	end
	tsort(list, function(a, b) return (a[2] or 0) > (b[2] or 0) end)
	...
	for i = 1, #list do
		local entry = list[i]
		if entry then
			local fname = "KRUmisdirectionCounter" .. entry[1]
...
function mod:OnInitialize()
	self.db = KRU.db.profile.misdirections
end
...
function mod:OnEnable()
	...
	if self.db.enabled then
		misdirections = self.db.misdirections
		ShowDisplay()
	else
		HideDisplay()
	end

	SLASH_KRUmisdirection1 = "/mis"
	SlashCmdList.KRUmisdirection = function(cmd)
