...
local L = KRU.L
local trick = GetSpellInfo(57934)
local mod = KRU:NewModule(trick)
...
KRU.defaults.profile.tricks = defaults
...
local AddTrick, ResetTricks, ReportTricks
local tricksData, trickFrames = {}, {}
local testTricks, testMode = {Crok = 20, Zaraza = 32, Ikiyu = 6, Vancleef = 12}

KRU.options.args.tricks = {
	type = "group",
	name = trick,
	order = 6,
	get = function(i)
	...
	args = {
		...
		reset = {
			...
			width = "double",
			confirm = function()
				return L:F("Are you sure you want to reset %s to default?", trick)
			end,
			func = function()
				KRU.db.profile.tricks = defaults
				mod.db = KRU.db.profile.tricks
				UpdateDisplay()
			end
		}
	}
...
function UpdateDisplay()
	...
	tricks = testMode and testTricks or mod.db.tricks

	for name, _ in pairs(tricks) do
		local f = _G["KRUTrickCounter" .. name]
		if f then
...
do
	...
	function CreateDisplay()
		...
		display = CreateFrame("Frame", "KRUTrickCounter", UIParent)
		...
		t:SetScript("OnMouseUp", function(self, button)
			if not testMode and next(tricks) and button == "RightButton" then
				menuFrame =
					menuFrame or CreateFrame("Frame", "KRUTrickCounterMenu", display, "UIDropDownMenuTemplate")
...
function AddTrick(name)
	tricks[name] = (tricks[name] or 0) + 1
	rendered = nil
end
...
function ReportTricks()
	if testMode then
		return
	end

	local list = {}
	for name, count in pairs(tricks) do
...
do
	...
	local function OnEvent(self, event, _, eventtype, _, srcName, _, _, _, _, _, spellname)
		if not self or self ~= display or event ~= "COMBAT_LOG_EVENT_UNFILTERED" then
			return
		elseif srcName and KRU:CheckUnit(srcName) and eventtype == "SPELL_CAST_SUCCESS" and spellname and spellname == trick then
			AddTrick(srcName)
		end
	end
...
function ResetFrames()
	for k, v in pairs(trickFrames) do
		if _G[k] then
			_G[k]:Hide()
			_G[k] = nil
		end
...
function RenderDisplay()
	...
	local list = {}
	for name, count in pairs(tricks or {}) do
		tinsert(list, {name, count})
	end
	tsort(list, function(a, b) return (a[2] or 0) > (b[2] or 0) end)
	...
	for i = 1, #list do
		local entry = list[i]
		if entry then
			local fname = "KRUTrickCounter" .. entry[1]
...
function mod:OnEnable()
	...
	SLASH_KRUTRICK1 = "/tri"
	SlashCmdList.KRUTRICK = function(cmd)
