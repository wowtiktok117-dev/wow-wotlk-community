...
local module = ExRT:New("LootLink",ExRT.L.LootLink)
...
local function LootLink(linkAnyway)
	...
	for i=1,count do
		local sourceGUID = GetLootSourceInfo and GetLootSourceInfo(i) or nil
		if not sourceGUID then
			sourceGUID = UnitGUID("target")
		end
		if sourceGUID and (not module.db.cache[sourceGUID] or linkAnyway) then
			...
			if (linkAnyway or isFutureRaid) then
				...
				local _, _, quality, iLevel = GetItemInfo(itemLink or 0)
