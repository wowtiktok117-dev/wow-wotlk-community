...
function module:Enable()
	...
	if not ExRT.isWotLKOnly then
		module:RegisterEvents('CHALLENGE_MODE_START')
	end
	if ExRT.isClassic then
...
function module:Disable()
	...
	if not ExRT.isWotLKOnly then
		module:UnregisterEvents('CHALLENGE_MODE_START')
	end
	if ExRT.isClassic then
...
local function GetCurrentMapForLogging()
	if VMRT.Logging.enabled then
		...
		elseif ExRT.isBC and VMRT.Logging.enableclassic5pplNormal and (difficulty == 173) then
			return true
		elseif ExRT.isWotLKOnly and VMRT.Logging.enableclassic5pplHC and zoneType == 'party' and difficulty == 2 then
			return true
		elseif ExRT.isWotLKOnly and VMRT.Logging.enableclassic5pplNormal and zoneType == 'party' and difficulty == 1 then
			return true
		end
	end
	return false
