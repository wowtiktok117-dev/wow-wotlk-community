...
module.db.diffNames = {
	[14] = L.sencounterWODNormal,
	[15] = L.sencounterWODHeroic,
	[16] = L.sencounterWODMythic,
	[3] = "10 ppl",
	[4] = "25 ppl",
	[5] = "10 ppl hc",
	[6] = "25 ppl hc",
...
function module.options:Load()
	...
	do
		...
		if ExRT.isWotLKOnly then
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[175],func = diff_SetVaule, arg1 = 175})
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[193],func = diff_SetVaule, arg1 = 193})
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[176],func = diff_SetVaule, arg1 = 176})
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[194],func = diff_SetVaule, arg1 = 194})
		else
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[14],func = diff_SetVaule, arg1 = 14})
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[15],func = diff_SetVaule, arg1 = 15})
			tinsert(self.EditPage.diff.List, {text = module.db.diffNames[16],func = diff_SetVaule, arg1 = 16})
		end
		tinsert(self.EditPage.diff.List, {text = L.minimapmenuclose,func = ELib.ScrollDropDown.Close})
	...
	self.sortBy1 = ELib:Radio(self,L.AttendanceSortByAny or "Any group",VMRT.Attendance.OptionsSortPD):Point("TOPLEFT",self.stats,"TOPRIGHT",5,-20):Tooltip(L.AttendanceSortByAnyTooltip or "Sort by attendance count in any raid group (columns 2 and 3)"):OnClick(function()
		VMRT.Attendance.OptionsSortPD = true
		self.sortBy1:SetChecked(true)
		self.sortBy2:SetChecked(false)
		UpdatePersonalData()
	end)
	self.sortBy2 = ELib:Radio(self,L.AttendanceSortByMain or "Main raid",not VMRT.Attendance.OptionsSortPD):Point("TOPLEFT",self.stats,"TOPRIGHT",5,-40):Tooltip(L.AttendanceSortByMainTooltip or "Sort by attendance count in main groups (columns 4 and 5)"):OnClick(function()
...
local CLASSIC_DIFF_WHITELIST = ExRT.isWotLKOnly and {175, 193, 176, 194} or {9, 148, 175, 176, 193, 194}
local function isAllowedClassicDiff(difficultyID)
	for i = 1, #CLASSIC_DIFF_WHITELIST do
		if CLASSIC_DIFF_WHITELIST[i] == difficultyID then
			return true
		end
	end
	return false
...
local function EncounterStartLog(encounterID, encounterName, difficultyID, groupSize)
	if not isAllowedClassicDiff(difficultyID) then
		return
	end
...
local function EncounterEndLog(encounterID, encounterName, difficultyID, groupSize, isKill)
	if not (isKill == 1) or not isAllowedClassicDiff(difficultyID) then
		return
	end
