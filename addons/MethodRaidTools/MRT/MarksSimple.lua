...
function module.options:Load()
	...
	local function Tab3LineUpdate(self,isFull)
		...
		if not spellID then
			...
			local spellTexture = ExRT.F.GetSpellTextureSafe and ExRT.F.GetSpellTextureSafe(spellID) or GetSpellTexture(spellID)
			self.texture:SetTexture(spellTexture or "Interface\\Icons\\INV_MISC_QUESTIONMARK")
			self.text:SetText(spellName or tostring(spellID))
		end
...
local function placeOnTargetOrMouseover(unit)
	...
	if guid and not guidToMark[guid] then
		...
		if not mobID and #guid >= 12 and not guid:find("-") then
			local cidDec = tonumber(guid:sub(8, 12), 16)
			if cidDec and cidDec > 0 then
				mobID = tostring(cidDec)
			end
		end
...
function module.main.COMBAT_LOG_EVENT_UNFILTERED(...)
	local _, event, _, _, _, _, _, _, destName, _, destRaidFlags, spellId = ...
	...
	if (event == "SPELL_AURA_APPLIED" or event == "SPELL_AURA_APPLIED_DOSE") then
		if data_buffs[ spellId ] and VMRT_MarksSimple_buffMarkEnabled then
			...
			if unit and GetRaidTargetIndex(unit) ~= currentMark then
				SetRaidTarget(unit, currentMark)
			end
	...
	elseif event == "UNIT_DIED" then
		if (destRaidFlags or 0) > 0 and VMRT_MarksSimple_autoMarkEnabled and not FIX_NOT_UNIT_DEATH_ENCOUNTER then
