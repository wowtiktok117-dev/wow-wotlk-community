...
module.db.diffNames = {
	...
	[148] = GetDifficultyInfo and GetDifficultyInfo(148) or "20ppl raid",
	[175] = GetDifficultyInfo and GetDifficultyInfo(175) or "10 ppl",
	[176] = GetDifficultyInfo and GetDifficultyInfo(176) or "25 ppl",
	[186] = GetDifficultyInfo and GetDifficultyInfo(186) or "40 ppl",
...
module.db.diffPos = ExRT.isWotLKOnly and {1,175,193,176,194} or ExRT.isLK and {1,148,9,3,4,175,176,193,194} or ExRT.isBC and {1,148,9,3,4,175,176} or ExRT.isClassic and {1,148,9,185,186} or {24,1,2,23,8,9,3,4,5,6,7,14,15,16}
...
function module.options:Load()
		...
		if not module.db.chachedDB then
					...
					for i=1,#playerData do
						...
						if diffID == newDiff then
							...
							if isKill then
								...
								if not pullTime or pullTime >= minPullTime then
									encounterLine.pulls = encounterLine.pulls + 1
								end
							end
			...
			for _,encounterData in pairs(encounters) do
				...
				for i=1,#encounterData.pullTable do
					if not encounterData.pullTable[i].d or encounterData.pullTable[i].d >= minPullTime then
	...
	for i=0,31 do
		...
		if i>0 then
			...
			line.pullClick:SetAllPoints(line)
			...
			local baseLevel = line:GetFrameLevel() or 0
			line.firstBloodB:SetFrameLevel(baseLevel + 5)
			line.pullClick:SetFrameLevel(baseLevel + 5)
...
function module.main:ADDON_LOADED()
	...
	if not VMRT.Encounter.MigratedV2 and VMRT.Addon.Version < 2022 then
		local alreadyNew = false
		for k, v in pairs(VMRT.Encounter.list) do
			if type(k) == "string" and not tonumber(k) and type(v) == "table" then
				alreadyNew = true
			end
			break
		end
		if not alreadyNew then
			local newTable = {}
			local newTableNames = {}
			for encID,encData in pairs(VMRT.Encounter.list) do
				if type(encData) == "table" then
					...
					for diffID,diffData in pairs(encData) do
						if tonumber(diffID) and type(diffData) == "table" then
							local diffHex = ExRT.F.tohex(diffID - 1)
							for _,pullData in pairs(diffData) do
								if type(pullData) == "table" and pullData.pull then
					...
					do local __id = tonumber(encHex,16) if __id then newTableNames[__id] = encData.name or "Unknown" end end
				end
			end
			VMRT.Encounter.list = newTable
			VMRT.Encounter.names = newTableNames
		end
		VMRT.Encounter.MigratedV2 = true
	end
...
function module.main:ENCOUNTER_START(encounterID, encounterName, difficultyID, groupSize)
	if difficultyID == 17 or difficultyID == 151 then
		difficultyID = 7
	end
