# GTFO | 📥 [Download / Descargar](https://github.com/wowtiktok117-dev/wow-wotlk-community/raw/main/addons/GTFO/GTFO.zip)

> Addon oficial verificado por **Wowtiktok WotLK Community**.
>
> Compatible con **World of Warcraft: Wrath of the Lich King 3.3.5a (Warmane)**.

---

| Campo | Valor |
|--------|-------|
| Estado | ✅ Mantenido |
| Compatibilidad | Warmane WotLK 3.3.5a |
| Categoría | Raid |
| Clasificación | Esencial |
| Prioridad de instalación | ⭐⭐⭐⭐⭐ |
| Contenido incluido | Archivos originales del addon |
| Última revisión | 2026-07-08 |

---

## 🎯 ¿Por qué lo recomendamos?

GTFO es un addon diseñado para alertar inmediatamente al jugador cuando permanece sobre zonas peligrosas o recibe daño evitable durante un encuentro.

Su objetivo es complementar los avisos de addons como **Deadly Boss Mods (DBM)** mediante alertas sonoras claras e inmediatas, ayudando a reaccionar con mayor rapidez y reduciendo errores durante las bandas.

En **Wowtiktok WotLK Community**, **GTFO se considera un addon esencial para participar en las raids oficiales**.

---

## 👥 ¿Quién debería utilizarlo?

Todos los jugadores que participen en contenido PvE organizado.

Especialmente:

- Tanques
- Sanadores
- DPS
- Raid Leaders

---

## ⚔️ ¿Cuándo resulta especialmente útil?

Durante cualquier encuentro en el que permanecer sobre zonas peligrosas, efectos de área o mecánicas evitables pueda provocar daño innecesario o comprometer el progreso de la banda.

---

## 📂 Contenido incluido

Esta carpeta contiene los archivos originales del addon **GTFO** sin modificaciones.

> Se respeta la estructura y el contenido originales distribuidos por el autor.

---

## 📦 Instalación

1. Cierra **World of Warcraft**.

2. Si ya tienes una versión anterior de **GTFO** instalada, elimina completamente su carpeta de:

```text
World of Warcraft/
└── Interface/
    └── AddOns/
```

3. Copia la carpeta **GTFO** dentro de:

```text
World of Warcraft/
└── Interface/
    └── AddOns/
```

4. Inicia el juego.

> **Recomendación de Wowtiktok WotLK Community**
>
> Para evitar errores, conflictos entre versiones o archivos obsoletos, recomendamos **no sobrescribir una instalación anterior**. Elimina primero la versión antigua e instala siempre el addon desde cero.
>
> La configuración predeterminada de GTFO es suficiente para comenzar a utilizarlo y puede personalizarse posteriormente desde el propio juego.

---

## 💡 Recomendación del equipo

Para obtener la mejor experiencia durante las bandas PvE, recomendamos utilizar **GTFO** junto con **Deadly Boss Mods (DBM)**.

Mientras **DBM** avisa de las mecánicas del encuentro mediante temporizadores y alertas, **GTFO** alerta inmediatamente cuando el jugador permanece sobre zonas peligrosas del suelo.

La combinación de ambos addons ayuda a reducir significativamente los errores durante los combates y mejora la capacidad de reacción de toda la banda.

---

## 🔗 Fuentes

### Autor

Zensunim of Malygos

### Versión verificada

2.5.3

---

## ❤️ Wowtiktok WotLK Community

Este addon forma parte de la colección oficial de **Wowtiktok WotLK Community**, un proyecto orientado a ofrecer una experiencia PvE organizada, mantenible y verificada para **World of Warcraft: Wrath of the Lich King 3.3.5a (Warmane)**.

### ¿Has encontrado un problema, una versión más reciente o tienes alguna sugerencia?

Puedes abrir una incidencia en el repositorio de GitHub o crear un ticket en nuestro servidor de Discord.

Revisaremos toda la información y, si es compatible con la filosofía del proyecto, la verificaremos y la incluiremos en una futura actualización.